...
### 0.10.5 發布前補充：解除本機 Google 綁定

新增明確入口與確認說明，保留本機筆記、待同步內容與雲端資料。進一步核對發現桌面舊 `disconnect` 曾呼叫撤銷授權，而 Android 只清除本機憑證；0.10.5 統一改為本機解除，不呼叫 Google 撤銷端點。解除前停止排程並等待既有工作區同步退出；Android 以授權世代檢查拒絕較晚回來的旧授權結果，避免重新填回剛清除的憑證。

單元測試涵蓋離線解除不發出網路請求、保留資料／待傳佇列、等待進行中的工作，以及清除憑證失敗不回報成功。隔離瀏覽器驗證取消不解除、確認只執行一次、390px 排版與44px操作高度。此項不等於已完成所有真機的 Google 登入／撤銷流程驗收。
