import { getHiddenCardIds } from "../lib/activeContent";
...
export function HighlightsView() {
  ...
  const highlights = useLiveQuery(async () => {
    ...
    return db.highlights.orderBy("createdAt").reverse().filter((item) => !hidden.has(item.cardId)).limit(visibleLimit).toArray();
  }, [visibleLimit], []);
  const totalCount = useLiveQuery(async () => {
    const hidden = await getHiddenCardIds();
    return db.highlights.filter((item) => !hidden.has(item.cardId)).count();
  }, [], 0);
