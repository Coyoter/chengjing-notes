...
export function JournalView() {
  ...
  const tasks = useLiveQuery(async () => {
    ...
    return db.tasks.orderBy("dueAt").filter((task) => !hidden.has(task.id) && !task.done && !task.parentTaskId).limit(6).toArray();
  }, [], []);
  const { dayjsLocale, intlLocale, t } = useI18n();

  useEffect(() => {
    let cancelled = false;
    void getOrCreateJournal(journalDate).then((card) => { if (!cancelled) setJournalId(card.id); });
