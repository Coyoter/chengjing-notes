import { createCard, createFragment, db, moveCardToTrash, deleteCardPermanently, deleteBoardPermanently, deleteTag, deleteKnowledgeGroup, createTag, renameTag, createKnowledgeGroup, renameKnowledgeGroup } from "../db";
...
async function workspaceStatus() {
  ...
  return {
    ...
    batchActions: "chengjing_apply_actions supports the built-in action schema, including deletion. Cards and fragment aliases go to trash; board deletion removes related local graph edges. Use chengjing_list_records to enumerate every page; never treat search results as the whole workspace.",
...
async function manageMetadata(args: Record<string, unknown>) {
  ...
  if (operation === "update" && (!id || !(table === "fragments" ? await getCaptureCard(id) : await db.table(table).get(id)))) throw new Error("mcp-item-not-found");
  if (table === "tags") {
