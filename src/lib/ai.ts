...
export async function searchSpace(query: string, limit = 8, language = currentLanguage()) {
  ...
  const cards = await db.cards.where("searchTerms").anyOf(terms).distinct().filter(isVisibleCard).limit(500).toArray();
  return cards
    .map((card) => {
    ...
    .filter((entry) => entry.score > 0 && isVisibleCard(entry.card))
...
export async function contextForCard(cardId: string) {
  ...
  if (!card || (!isActiveCard(card) && useAppStore.getState().inactiveCardAccessId !== cardId)) return "";
...
export async function contextForBoard(boardId: string) {
  ...
  const cards = (await Promise.all(nodes.filter((node) => node.cardId).map((node) => db.cards.get(node.cardId!)))).filter(isActiveCard);
  const visibleCards = new Set(cards.map((card) => card.id));
  const visibleNodes = new Set(nodes.filter((node) => !node.cardId || visibleCards.has(node.cardId)).map((node) => node.id));
  const edges = await db.boardEdges.where("boardId").equals(boardId).filter((edge) => visibleNodes.has(edge.source) && visibleNodes.has(edge.target)).toArray();
  return [
