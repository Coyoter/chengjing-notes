  ...
  return (
    ...
    >
      {card && <>
        ...
        <button type="button" role="menuitem" data-menu-action="edit" onClick={() => run(() => useAppStore.getState().openCard(card.id, card.state === "archived" ? "archive" : card.state === "trash" ? "trash" : "library"))}><Pencil size={15} />{editCopy.edit}</button>
        ...
        {card.state !== "trash" && <button type="button" role="menuitem" data-menu-action="convert-board" onClick={() => run(() => { useAppStore.getState().openCard(card.id, card.state === "archived" ? "archive" : card.state === "trash" ? "trash" : "library"); useAppStore.getState().openAIWithAction(t("card.convertToBoardPrompt")); })}><PanelsTopLeft size={15} />{t("card.convertToBoard")}</button>}
      ...
      {fragment && <>
        ...
        <button type="button" role="menuitem" onClick={() => run(async () => { const card = await cardFromFragment(); if (card) useAppStore.getState().openCard(card.id, card.state === "archived" ? "archive" : card.state === "trash" ? "trash" : "library"); })}><FilePlus2 size={15} />{t("context.toCard")}</button>
