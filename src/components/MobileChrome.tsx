...
export function MobileChrome() {
  ...
  const [sheet, setSheet] = useState<"all" | "work" | null>(null);
  ...
  return <>
    <header className="mobile-header"><span>澄境<small>CHENGJING</small></span><div><button aria-label={t("top.searchPlaceholder")} onClick={() => useAppStore.getState().setCommandOpen(true)}><Search size={21} /></button><button aria-label={t("nav.primary")} onClick={()=>setSheet("all")}><Menu size={23}/></button></div></header>
    <nav className="mobile-nav" aria-label={t("nav.primary")}>
      ...
      <button className={["boards", "kanban", "database"].includes(view) ? "active" : ""} onClick={() => setSheet("work")}><LayoutGrid /><span>{t("top.workspace")}</span></button>
    </nav>
    <AnimatePresence>{sheet && <motion.div className="mobile-sheet-backdrop" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={() => setSheet(null)}><motion.section className="mobile-sheet mobile-directory" drag="y" dragControls={dragControls} dragListener={false} dragConstraints={{top:0,bottom:0}} dragElastic={{top:0,bottom:0.4}} onDragEnd={(_,info)=>{if(info.offset.y>80||info.velocity.y>500)setSheet(null)}} initial={reduced?false:{y:"100%"}} animate={{y:0}} exit={{y:"100%"}} transition={{type:"spring",stiffness:380,damping:36}} role="dialog" aria-modal="true" aria-label={t("nav.primary")} onClick={(event) => event.stopPropagation()}>
      <header onPointerDown={event=>dragControls.start(event)}><i className="mobile-sheet-handle"/><div><small>CHENGJING</small><strong>{zh?"你的工作空間":"Your workspace"}</strong></div><button aria-label={t("ai.close")} onClick={() => setSheet(null)}><X size={21}/></button></header>
