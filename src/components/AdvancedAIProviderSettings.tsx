...
import type { AIProviderModel, AIProviderProfile, AIProviderSettings, AIProviderType } from "../types";
...
export function AdvancedAIProviderSettings() {
  ...
  function loadProfile(profile: AIProviderProfile) {
    setEditingId(profile.id); setType(profile.type); setName(profile.name); setBaseUrl(profile.baseUrl); setModel(profile.model); setApiKey(""); setModels([]); setNotice("");
  }

  function resetForm(nextType: AIProviderType = "ollama") {
    setEditingId(""); setType(nextType); setName(nextType === "ollama" ? "Ollama" : "Custom Gateway"); setBaseUrl(nextType === "ollama" ? "http://127.0.0.1:11434/v1" : "https://"); setModel(""); setApiKey(""); setModels([]); setNotice("");
  }
  ...
  useEffect(() => { if (engine === "custom-provider" && settings.profiles.length === 0) setExpanded(true); }, [engine, settings.profiles.length]);
  ...
  async function saveConnection(event: React.FormEvent) {
    ...
    try {
      const value = await window.chengjing.ai.upsertProvider({ id: editingId || undefined, name, type, baseUrl, model, ...(apiKey.trim() ? { apiKey } : {}), select: true });
      setSettings(value);
  ...
  async function removeKey() {
    ...
    setBusy("save");
    try {
      const value = await window.chengjing.ai.upsertProvider({ id: editingProfile.id, name, type, baseUrl, model, apiKey: "", select: settings.selectedProfileId === editingProfile.id });
      ...
      <div className="advanced-provider-body">
        ...
        {settings.profiles.length > 0 && <div className="provider-profile-list">{settings.profiles.map((profile) => {
          ...
          return <article key={profile.id} className={active ? "is-active" : ""}>
            <button type="button" className="provider-profile-main" onClick={() => loadProfile(profile)}><span><b>{profile.name}</b><small>{profile.model}</small></span><code>{profile.type === "ollama" ? "Ollama" : "Gateway"}</code></button>
